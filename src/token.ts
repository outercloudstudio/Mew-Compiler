		...
		for (let i = 0; i < tokens.length; i++) {
			if (matchType(tokens[i], 'block')) {
				...
				for (let paramIndex = 0; paramIndex < tokens[i].content.params.length; paramIndex++) {
					tokens[i].content.params[paramIndex] = runInContexts(
						func,
						tokens[i].content.params[paramIndex],
						'block param'
					)
				}
				...
				matchType(tokens[i], 'definition') &&
				matchToken(tokens[i].content.word, 'descriptor', 'function')
			) {
				tokens[i].content.value.content.content = runInContexts(
					func,
					tokens[i].content.value.content.content,
					'function'
				)
