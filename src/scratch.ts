...
function handleInputConvert(
	value: string | Block | Reference | Stack,
...
function convertInput(value: string | Block | Reference | Stack, type?: number | null) {
	if (value instanceof Block) {
		return [3, value.id, [type, 'unset']]
	}

	if (value instanceof Reference) {
		return [3, [12, value.name, value.name], [type, 'unset']]
	}

	if (value instanceof Stack) {
		return [2, value.blocks.length > 0 ? value.blocks[0].id : null]
	}
...
export class DefinitionBlock extends Block {
	...
	convert(): object {
		...
		target[this.id] = {
			...
			inputs: {
				custom_block: [1, 'prototype-' + this.id],
			},
			fields: {},
			shadow: false,
...
export class SetVariableBlock extends Block {
	...
	convert(): object {
		...
		target[this.id] = {
			...
			fields: {
				VARIABLE: [this.variableName, this.variableName],
			},
			shadow: false,
			topLevel: this.parent == null,
		}
